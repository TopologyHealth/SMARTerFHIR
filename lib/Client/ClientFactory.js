 ...
 */
            ...
            return __generator(this, function (_a) {
                switch (launchType) {
                    ...
                    case LAUNCH.STANDALONE:
                        return [2 /*return*/, this.buildStandaloneFhirClient()];
                    default:
    ...
    ClientFactory.prototype.buildStandaloneFhirClient = function () {
        return __awaiter(this, void 0, void 0, function () {
            var code, _a, endpoints, clientId, redirectUri, tokenResponse, defaultFhirClient;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        code = getCodeFromBrowserUrl();
                        _a = this.getRequiredTokenParameters(code), endpoints = _a.endpoints, clientId = _a.clientId;
                        redirectUri = window.location.origin + window.location.pathname // The current URL minus any parameters
                        ;
                        return [4 /*yield*/, getAccessToken(endpoints.token, code, clientId, redirectUri)];
                    case 1:
                        tokenResponse = _b.sent();
                        defaultFhirClient = FHIR.client(endpoints.r4.toString());
    ...
    ClientFactory.prototype.getRequiredTokenParameters = function (code) {
        try {
            var decodedJwt = codeToJwt(code);
            return { endpoints: this.getEmrEndpoints(decodedJwt), clientId: decodedJwt.client_id };
        }
        catch (reason) {
            var clientIdFromEnv = process.env.REACT_APP_EMR_CLIENT_ID;
            var emrType = process.env.REACT_APP_EMR_TYPE.toLowerCase();
            var isRequiredParamsGiven = emrType && clientIdFromEnv;
            if (!isRequiredParamsGiven) {
                ...
                if (!emrType)
                    throw new Error('EMR type cannot be inferred. You must provide the emr_type explicitly');
 ...
 */
function getAccessToken(tokenEndpoint, code, clientId, redirectUri) {
    return __awaiter(this, void 0, void 0, function () {
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
