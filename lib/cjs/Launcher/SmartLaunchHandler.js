...
var SmartLaunchHandler = /** @class */ (function () {
    ...
    function SmartLaunchHandler(clientID, clientSecret, scope) {
        ...
        if (scope) {
            var hasCommaSeparators = scope.includes(",");
            if (!hasCommaSeparators)
                throw new Error("`scope` argument is of invalid format. `scope` must be provided as a string of comma-separated values");
            this.scopeOverride = scope.split(",").map(function (s) { return s.trim(); });
        }
    }
    /**
                ...
                switch (_b.label) {
                    case 0:
                        ...
                        if (emrType === undefined)
                            emrType = (0, utils_1.getEMRType)(new URL(iss));
                        if (emrType === EMR.NONE)
